...
use smithay::{
    backend::{
        allocator::{
            ...
            Swapchain,
            Modifier,
            gbm::{GbmDevice, GbmBuffer},
            dmabuf::{AsDmabuf, Dmabuf},
        },
        drm::{DrmNode, NodeType},
        egl::{EGLDisplay, EGLDevice, EGLContext},
        renderer::{
            Bind, Unbind,
            ImportMemWl,
            ImportDma,
    ...
    wayland::{
        ...
        compositor::{CompositorState, CompositorHandler, with_states, get_children},
        data_device::{DataDeviceState, DataDeviceHandler, ClientDndGrabHandler, ServerDndGrabHandler, set_data_device_focus},
        dmabuf::{DmabufState, DmabufHandler, DmabufGlobal, ImportError, get_dmabuf},
        output::{OutputManagerState, Output, Mode as OutputMode, PhysicalProperties, Scale},
        seat::{SeatState, SeatHandler, Seat, XkbConfig},
        shell::xdg::{XdgShellState, XdgShellHandler, ToplevelSurface, PopupSurface, PositionerState, XdgPopupSurfaceRoleAttributes, XdgToplevelSurfaceRoleAttributes},
        shm::{ShmHandler, ShmState},
        viewporter::ViewporterState,
        Serial,
        socket::ListeningSocketSource,
    },
    utils::{Point, Size, Logical, Transform},
    delegate_compositor,
...
use self::export_dmabuf::{
    ExportDmabufState,
    ExportDmabufHandler,
    Capture, CaptureError,
};
use self::gl_allocator::{GlAllocator, Renderbuffer};
use self::input::*;
use self::output_configuration::{
...
struct State {
    ...
    //swapchain: Swapchain<GlAllocator, Renderbuffer>,
    ...
    output_conf_state: OutputConfigurationState<Self>,
...
impl CompositorHandler for State {
    ...
    fn commit(&mut self, dh: &DisplayHandle, surface: &WlSurface) {
        ...
        if let Some(idx) = self.pending_windows.iter_mut().position(|w| w.toplevel().wl_surface() == surface)
        {
            ...
            if let SurfaceKind::Xdg(ref toplevel) = window.toplevel() {
                let (initial_configure_sent, max_size) = with_states(surface, |states| {
                    let attributes = states
                        ...
                        .unwrap();
                    let attributes_guard = attributes
                        .lock()
                        .unwrap();
                ...
                if !initial_configure_sent {
                    if max_size.w == 0 && max_size.h == 0 {
                        toplevel.with_pending_state(|state| {
                            state.size = Some(
                                self.output.current_mode().unwrap().size
                                .to_f64()
...
impl DmabufHandler for State {
    ...
    fn dmabuf_imported(
        ...
        self.renderer.import_dmabuf(&dmabuf, None)
            .map(|_| ())
...
impl OutputConfigurationHandler for State {
    fn output_configuration_state(&mut self) -> &mut OutputConfigurationState<Self> {
        &mut self.output_conf_state
    }

    fn test_configuration(&mut self, mut conf: Vec<(Output, OutputConfiguration)>) -> bool {
        match conf.remove(0).1 {
            // you cannot remove our only output
            OutputConfiguration::Disabled => return false,
            OutputConfiguration::Enabled {
                position,
                ..
            } => {
                // you cannot move our only output
                if position.map(|p| p.x != 0 || p.y != 0).unwrap_or(false) {
                    return false;
                }
    ...
    fn apply_configuration(&mut self, mut conf: Vec<(Output, OutputConfiguration)>) -> bool {
        match conf.remove(0).1 {
            // you cannot remove our only output
            OutputConfiguration::Disabled => return false,
            OutputConfiguration::Enabled {
                ...
                let mode = mode.map(|m| match m {
                    ModeConfiguration::Mode(mode) => mode,
                    ModeConfiguration::Custom {
                        size,
                        refresh,
                    } => {
                        OutputMode {
                            size,
                            refresh: refresh.unwrap_or(60_000),
                        }
                    }
                });
...
impl ExportDmabufHandler for State {
    fn capture_frame(&mut self, dh: &DisplayHandle, _output: WlOutput, overlay_cursor: bool) -> Result<Capture, CaptureError> {
        self.space.send_frames(self.start_time.elapsed().as_millis() as u32);
        ...
        */
        ...
        let buffer_size = physical_output_size.to_logical(1).to_buffer(1, Transform::Normal);

        let elements = if overlay_cursor {
            self.cursor_element.set_location(self.pointer_location.to_physical_precise_round(self.output.current_scale().fractional_scale()));
            vec![self.cursor_element.clone()]
        } else { Vec::new() };

        let offscreen = self.swapchain.acquire().map_err(|err| CaptureError::Temporary(Box::new(err)))?.unwrap();
        ...
        if dmabuf.is_none() {
            ...
            offscreen.userdata().insert_if_missing(|| new_dmabuf.clone());
            dmabuf = Some(new_dmabuf);
        }
        let dmabuf = dmabuf.unwrap();
        self.renderer.bind(dmabuf).map_err(|err| CaptureError::Temporary(Box::new(err)))?;

        self.space.render_output(
            &dh,
            &mut self.renderer,
            &self.output,
            age as usize,
        ...
        let res = self.renderer.export_framebuffer(buffer_size)
            .map(|dmabuf| Capture {
                dmabuf: dbg!(dmabuf),
                presentation_time: std::time::Instant::now(),
            })
            .map_err(|err| CaptureError::Temporary(Box::new(err)))?;
        self.renderer.unbind().map_err(|err| CaptureError::Temporary(Box::new(err)))?;
        Ok(res)
    }
...
struct Args {
    #[clap(short = 'r', long, value_parser, default_value_t = String::from("640x480"))]
    resolution: String,
    #[clap(short = 'f', long, value_parser, default_value_t = 60)]
    framerate: u32,
    #[clap(short = 'd', long, value_parser)]
    device_path: String,
    #[clap(short = 's', long, value_parser, default_value_t = String::from("seat-0"))]
    input_seat: String,
}
...
fn main() -> smithay::reexports::calloop::Result<()> {
    ...
    let args = Args::parse();
    let (w, h) = args.resolution.split_once("x")
        .expect("resolution should be in format <W>x<H>");
    let size = (
        w.parse::<u32>().expect(&format!("{} is no valid integer", w)) as i32,
        h.parse::<u32>().expect(&format!("{} is no valid integer", h)) as i32,
    );

    let log = ::slog::Logger::root(slog_term::FullFormat::new(
    ...
    let mut output_conf_state = OutputConfigurationState::<State>::new(&dh, |_| true);
    ...
    */
    ...
    let drm_node = std::fs::File::open(user_node.dev_path_with_type(NodeType::Render)
    ...
    let _guard = egl.bind_wl_display(&dh).expect("Failed to bind egl display");
    ...
    */
    ...
    let modifiers = context.dmabuf_texture_formats().into_iter().filter(|x| x.code == Fourcc::Nv12).map(|x| x.modifier).collect();
    let swapchain = Swapchain::new(allocator, size.0 as u32, size.1 as u32, Fourcc::Nv12, modifiers);
    let mut renderer =
        unsafe { Gles2Renderer::new(context, log.clone()) }.expect("Failed to initialize renderer");
    let formats = Bind::<Dmabuf>::supported_formats(&renderer).expect("Failed to query formats").into_iter().collect::<Vec<_>>();
    ...
    let _drm_global = drm_state.create_global::<State>(&dh, std::path::PathBuf::from(&args.device_path), formats, &dmabuf_global);
    let cursor_element = CursorElement::new(&mut renderer, (size.0 as f64 / 2.0, size.1 as f64 / 2.0));

    // init input backend
    let mut libinput_context = Libinput::new_with_udev(NixInterface);
    libinput_context.udev_assign_seat(&args.input_seat).expect("Failed to assign libinput seat");
    let libinput_backend = LibinputInputBackend::new(
        libinput_context,
        log.clone(),
    );

    // init wayland objects
    let output = Output::new("HEADLESS-1".into(), PhysicalProperties {
        make: "Virtual".into(),
        model: "Sunrise".into(),
        size: (0, 0).into(),
    ...
    slog::info!(log, "Listening on wayland socket: {}", source.socket_name().to_string_lossy());
    event_loop
        .handle()
    ...
    loop {
        ...
        data.display.flush_clients().expect("Failed to flush clients");
    }
}
